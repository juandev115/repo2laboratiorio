Algoritmo CalculoRetrasoTareas
    ...
    Definir es_bisiesto, es_bisiesto2 Como Logico
    ...
    // --- Convertir cada fecha a número ordinal ---
    leaps_antes <- (anio - 1) DIV 4 - (anio - 1) DIV 100 + (anio - 1) DIV 400
    dias_mes_antes <- 0
    m <- 1
    ...
    leaps_antes2 <- (anio2 - 1) DIV 4 - (anio2 - 1) DIV 100 + (anio2 - 1) DIV 400
    dias_mes_antes2 <- 0
    m <- 1
